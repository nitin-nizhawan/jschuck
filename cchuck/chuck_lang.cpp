...
CK_DLL_CTOR( HidIn_ctor )
{
   /* HidIn * min = new HidIn;
    min->SELF = SELF;*/
    OBJ_MEMBER_INT(SELF, HidIn_offset_data) = (t_CKINT)NULL;//min;
}
...
CK_DLL_MFUN( HidIn_open )
{
  /*  HidIn * min = (HidIn *)OBJ_MEMBER_INT(SELF, HidIn_offset_data);
    ...
    RETURN->v_int = 1;//min->open( type, num );
}
...
CK_DLL_MFUN( HidIn_open_named )
{
   /* HidIn * min = (HidIn *)OBJ_MEMBER_INT(SELF, HidIn_offset_data);
    Chuck_String * name = GET_NEXT_STRING(ARGS);*/
    RETURN->v_int = 1;//min->open( name->str );
}
...
CK_DLL_MFUN( HidIn_open_joystick )
{
    /*HidIn * min = (HidIn *)OBJ_MEMBER_INT(SELF, HidIn_offset_data);
    t_CKINT num = GET_NEXT_INT(ARGS);*/
    RETURN->v_int = 1;//min->open( CK_HID_DEV_JOYSTICK, num );
}
...
CK_DLL_MFUN( HidIn_open_mouse )
{
    /*HidIn * min = (HidIn *)OBJ_MEMBER_INT(SELF, HidIn_offset_data);
    t_CKINT num = GET_NEXT_INT(ARGS);*/
    RETURN->v_int = 1;//min->open( CK_HID_DEV_MOUSE, num );
}
...
CK_DLL_MFUN( HidIn_open_keyboard )
{
   /* HidIn * min = (HidIn *)OBJ_MEMBER_INT(SELF, HidIn_offset_data);
    t_CKINT num = GET_NEXT_INT(ARGS);*/
    RETURN->v_int = 1;//min->open( CK_HID_DEV_KEYBOARD, num );
}

CK_DLL_MFUN( HidIn_open_tiltsensor )
{
   /* HidIn * min = (HidIn *)OBJ_MEMBER_INT(SELF, HidIn_offset_data);*/
    RETURN->v_int = 1;//min->open( CK_HID_DEV_TILTSENSOR, 0 );
}

CK_DLL_MFUN( HidIn_good )
{
//    HidIn * min = (HidIn *)OBJ_MEMBER_INT(SELF, HidIn_offset_data);
    RETURN->v_int = 1;//(t_CKINT)min->good();
}

CK_DLL_MFUN( HidIn_num )
{
  //  HidIn * min = (HidIn *)OBJ_MEMBER_INT(SELF, HidIn_offset_data);
    RETURN->v_int = 1;//min->num();
}
...
CK_DLL_MFUN( HidIn_name )
{
    ...
    RETURN->v_string->str = NULL;//min->name();
}
...
CK_DLL_MFUN( HidIn_recv )
{
   ...
   // Chuck_Object * fake_msg = GET_CK_OBJECT(ARGS);
  //  HidMsg the_msg;
    RETURN->v_int = 1;//min->recv( &the_msg );
/*
    if( RETURN->v_int )
    {
...
/*    t_CKINT type = GET_NEXT_INT(ARGS);
    ...
    RETURN->v_int = 1;//min->read( type, num, &the_msg );
    /*
    if( RETURN->v_int )
    {
        OBJ_MEMBER_INT(fake_msg, HidMsg_offset_device_type) = the_msg.device_type;
...
CK_DLL_MFUN( HidIn_send )
{
    ...
    */
    RETURN->v_int = 1;//min->send( &the_msg );
}

CK_DLL_MFUN( HidIn_can_wait )
{
//    HidIn * min = (HidIn *)OBJ_MEMBER_INT(SELF, HidIn_offset_data);
    RETURN->v_int = 1;//min->empty();
}
...
{
    RETURN->v_int = 0;//!Mouse_start_cursor_track();
}

CK_DLL_SFUN( HidIn_stop_cursor_track )
{
    RETURN->v_int = 0;//!Mouse_stop_cursor_track();
}
...
{
    OBJ_MEMBER_INT(SELF, HidOut_offset_data) = (t_CKUINT)NULL;//new HidOut;
}
...
CK_DLL_MFUN( HidOut_open )
{
    ...
    RETURN->v_int = 1;//mout->open( num );
}

CK_DLL_MFUN( HidOut_good )
{
//    HidOut * mout = (HidOut *)OBJ_MEMBER_INT(SELF, HidOut_offset_data);
    RETURN->v_int = 1;//(t_CKINT)mout->good();
}

CK_DLL_MFUN( HidOut_num )
{
   // HidOut * mout = (HidOut *)OBJ_MEMBER_INT(SELF, HidOut_offset_data);
    RETURN->v_int = 1;//mout->num();
}
...
CK_DLL_MFUN( HidOut_send )
{
  ...
  //  HidMsg the_msg;
    ...
    RETURN->v_int = 1;//mout->send( &the_msg );
}
